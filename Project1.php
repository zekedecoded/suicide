...
class Wallet
{
    ...
    public function getTransactions()
    {
        ...
        JOIN users su ON w.user_id = su.userID
    ...
    {
        $stmt = $this->con->prepare("SELECT COUNT(*) as totalStudents FROM users WHERE roleID = 2");
        $stmt->execute();
        if (!$stmt->rowCount()) {
            return [];
        }
    ...
    {
        $stmt = $this->con->prepare("SELECT COUNT(*) as totalMerchant FROM users WHERE roleID = 3");
        $stmt->execute();
        if (!$stmt->rowCount()) {
            return [];
        }
    ...
    {
        $stmt = $this->con->prepare("SELECT COUNT(*) as totalTransactions FROM transaction");
        $stmt->execute();
        if (!$stmt->rowCount()) {
            return [];
        }
    ...
    {
        $stmt = $this->con->prepare("SELECT SUM(amount) as totalVolume FROM transaction WHERE DATE(date_time) = CURDATE()");
        $stmt->execute();
        if (!$stmt->rowCount()) {
            return [];
        }
